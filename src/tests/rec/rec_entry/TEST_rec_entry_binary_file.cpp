...
TEST(rec_entry_binary_files_tests, rec_entry_binary_files_tests) {
    ...
    ASSERT_TRUE(to_file(rec_entry, "bin/test_rec_entry.bin"));

    rec_entry_t parsed_rec_entry(rec_template);

    ASSERT_TRUE(from_file("bin/test_rec_entry.bin", parsed_rec_entry));
